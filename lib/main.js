...
function updateAppStats( app ) {
	...
	// Update app stats
	if (app.status == 'up') {
		pidusage(app.pid, function (err, stats) {
			//console.log(stats)
			if (err==null) {
				let logicalProcessors = os.cpus().length
				app.stats.cpu = stats.cpu/logicalProcessors
				app.stats.memory = stats.memory
		...
		//app.stats.memory = result.memory //? result.memory : 0//process.memoryUsage.rss//
...
function load ( params ) {
	...
	var json = Buffer.from(params.jsonFile.data)
	/*try {
...
async function add ( params ) {
	...
	let stream = docker.buildImage('D:\\lab1servlags.tar', {t: "lab1servlags"}, function (err, response){
		if(!err){
	...
	await common.firestore.collection("users").doc(params.uid).update({
		apps: common.FieldValue.arrayUnion(params.app.name)
	}).then(() => {
...
async function set ( params ) {
	...
	if (options.name!=null && options.name!=app.name){
		await common.firestore.collection("users").doc(params.uid).update({
			apps: common.FieldValue.arrayRemove(app.name)
		}).then(() => {
			console.log("Document updated ");
		})
			.catch(function (error) {
		...
		await common.firestore.collection("users").doc(params.uid).update({
			apps: common.FieldValue.arrayUnion(options.name)
		}).then(() => {
			console.log("Document updated ");
		})
			.catch(function (error) {
...
async function  getDocument(uid){
	var doc;
	await common.firestore.collection("users").doc(uid).get()
		.then((document) => {
			doc = document
		})
		.catch(function (error) {
	...
	/*	var doc = await getDocument(params.uid)
		if (doc.exists) {
			if (doc.data()["role"] == "admin") {
				...
				var userApps = []
				doc.data()["apps"].forEach(appname => {
						var app = getAppByName(appname)
						if (app != null)
							userApps.push(app)
					}
				)
				return getSuccess(userApps)
			}
		} else {
			return getError("Error getting documents")
		}*/
	}
...
//'65953517b7b87957ef8c52f6d91d1bb4c4be94aa3023c396990235c69f7a3ba0'
//('1004ab591a84df9774afec92db4fc128c289914c18907d1c40f8adc135ba4d03')
...
async function monit ( params ) {
	...
	var necessaryContainers = containersId.filter(function(id) {
		if(activeContainers.includes(id))
		return id;
	});
	return getSuccess(
		await getStatsContainers(necessaryContainers) )
	...
	var stats = []
	var appsName = params.appsName
	for (var a=0; a<apps.length; a++) {
		var app = apps[a]
		if (appsName.indexOf(app.name)>-1  && app.status == 'up') {
			updateAppStats( app )
			stats.push({
				name: app.name,
				pid: app.pid,
				cpu: app.stats.cpu,
				mem: app.stats.memory//process.memoryUsage.rss//
			})		
		}	
	}
...
function startApp ( app ) {
	...
	if (app.keep) {
		child.proc.on('error', function (code) {
			  ...
			  getEmailsToSend(app.name, 0).then(emails=>
			  {
			  //	emails.forEach(email=>{
					SendMail('App ' + app.name + ' crashed with code (' + code + ')' + app.name, emails)
				//})
		...
		child.proc.on('close', function (code, signal) {
			...
			getEmailsToSend(app.name, 1).then(emails=>
			{
				//emails.forEach(email=>{
					SendMail('App ' + app.name + ' closed with (code:' + code + ') and (signal:' + signal + ')', emails)
				//})
		...
		child.proc.on('exit', function (code) {
			...
			getEmailsToSend(app.name, 2).then(emails=>
			{
			//	emails.forEach(email=>{
					SendMail('App ' + app.name + ' exited with code (' + code + ')', emails)
			//	})
...
async function startAll ( params ) {
	...
	var options = getObjectFromOptions( params.options )
	//console.log( 'options', options )
	var doc = await getDocument(options.uid)
	var appCount = 0
	for (var a=0; a<apps.length; a++) {
		var app = apps[a]
		if ((doc.data()["role"] == "admin" || doc.data()["apps"].indexOf(app.name)>-1) && (app.status == 'down' &&
			app.name != 'master' &&
			app.enabled)) {
...
async function stopAll ( params ) {
	...
	var options = getObjectFromOptions( params.options )
	//console.log( 'options', options )
	if (params.itsServer!=null && params.itsServer =="server"){
		var appCount = 0
		for (var a = 0; a < apps.length; a++) {
			var app = apps[a]
			if ((app.status == 'up' &&
		...
		for (var a = 0; a < apps.length; a++) {
			var app = apps[a]
			if ((doc.data()["role"] == "admin" || doc.data()["apps"].indexOf(app.name) > -1) && (app.status == 'up' &&
				app.name != 'master' &&
				app.enabled)) {
...
async function getListGroup(params){
	var doc = await getDocument(params.uid)
	var group=[]
	if (doc.exists) {
		if (doc.data()["role"] == "admin") {
			apps.forEach(elem => {
					if (group.indexOf(elem.group) === -1) {
						group.push(elem.group)
					}
				}
			)
		} else {
			doc.data()["apps"].forEach( appname=>
				{
					var app = getAppByName(appname)
					if (app!=null && group.indexOf(app.group) === -1)
						group.push(app.group)
				}
			)
		}
	}else {return  getError("Error getting documents")}

	var message = 'Groups sent successfully.'
	return getSuccess( group, message )
}
...
module.exports.getListGroup = getListGroup
module.exports.GetSettings = GetSettings
module.exports.SetExitSend = SetExitSend
module.exports.SetCloseSend = SetCloseSend
module.exports.SetCrashSend = SetCrashSend
